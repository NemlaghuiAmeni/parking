...
Reservation(){
  // this.adminId= this.adminId.substr(1,this.userId.length-2)
this.userId= this.userId.substr(1,this.userId.length-2)

 console.log("userID from Map: "+this.userId)
   
//console.log("admin Id from map to reservation: "+this.adminId)
  const data={park:this.parkSelected,place:this.placeSelected,dateD:this.reserv.dateE,dateF:this.reserv.dateS,timeD: this.reserv.timeE
 ,timeF:this.reserv.timeS,matricule:this.reserv.matricule,userId:this.userId ,adminId:this.adminId}
  console.log("park: "+data.park+" place: "+data.place+ "mat: "+data.matricule)
this.parkService.makeReservation(data).subscribe(data=>{console.log("reservation data after : "+data)})
